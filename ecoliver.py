...
def ar1simulate(a, sig_eps, T, x0=0, burnin=100):
    '''
    Simulate an order-1 autoregressive process, given input coefficients
     x(t+1) = a * x(t) + eps(t)
    where eps(t) ~ N(0, sig_eps**2)
    T indicates the length of the resulting time series.
    x0 is the intial value, x(0), and assumes = 0 by default.
    burnin is the number of samples to generate, and reject before saving the remaining data.
    '''
    # Generate noise forcing (eps)
    eps = sig_eps*np.random.randn(T+burnin)
    # Initialise timeseries (x)
    x = np.zeros(eps.shape)
    x[0] = x0
    # Loop and generate time series
    for i in range(1, len(x)):
        x[i] = a*x[i-1] + eps[i]
    # Output result, rejecting burn-in period
    return x[burnin:]

def ellipse(a, b, N=360):
    '''
    Returns the coordinates (x,y) that defines an ellipse.
    a and b are the semimajor (x) and semiminor (y) axes, respectively.
    N is the number of points (i.e. angles) of the returned array.
    It is assumed the ellipse is centred on (0,0), and without inclination.
    '''
    theta_deg = np.linspace(0., 360., N)
    theta = theta_deg*np.pi/180.
    x = np.zeros(theta_deg.shape)
    y = np.zeros(theta_deg.shape)
    for tt in range(N):
        den = np.sqrt(b**2 + (a*np.tan(theta[tt]))**2)
        if (theta_deg[tt] < 90.) + (theta_deg[tt] > 270.):
            x[tt] = a*b/den
            y[tt] = a*b*np.tan(theta[tt])/den
        else:
            x[tt] = -1.*a*b/den
            y[tt] = -1.*a*b*np.tan(theta[tt])/den
    #
    return x, y
