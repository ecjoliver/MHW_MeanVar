...
def simulate(t, sst_obs, seas_obs, sst_trend_obs, sst_trend_var, N_ens, params=None):
    '''
    ...
    keys = ['count', 'intensity_mean', 'duration', 'intensity_max_max', 'total_days']
    N_keys = len(keys)
    trends = {}
    means = {}
    for key in keys:
    ...
    for i_ens in range(N_ens): 
        ...
        # Add trend to the mean and variance
        sst_trend_var_applied = sst_trend_var / (2*np.var(sst))
        #sst = sst * (1 + np.sqrt(sst_trend_var_applied*(t-t[0])/10./365.25))
        sst = sst * (1 + np.sign(sst_trend_var_applied)*np.sqrt(np.abs(sst_trend_var_applied)*(t-t[0])/10./365.25))
        sst = sst + sst_trend_obs*(t-t[0])/10./365.25
        # Apply Marine Heat Wave definition
        mhws, clim = mhw.detect(t, sst)
        mhwBlock = mhw.blockAverage(t, mhws)
        #mean, trend, dtrend = meanTrend_TS(mhwBlock)
